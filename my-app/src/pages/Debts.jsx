...
const Debt = () => {
  const [debts, setDebts] = useState([]);
  const [form, setForm] = useState({ name: "", type: "credit", amount: "", date: "" });
  const [editingId, setEditingId] = useState(null);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (editingId) {
      await axios.put(`https://monatour-3.onrender.com/api/debt/${editingId}`, form);
    } else {
      await axios.post("https://monatour-3.onrender.com/api/debt", form);
    }
    setForm({ name: "", type: "credit", amount: "", date: "" });
    setEditingId(null);
    fetchDebts();
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6">
      <h1 className="text-2xl sm:text-3xl font-bold mb-4">الدائن والمدين</h1>
